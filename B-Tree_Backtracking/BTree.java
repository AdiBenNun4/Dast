...
public class BTree<T extends Comparable<T>> {
	...
	/*contains by pop order:
	 * last inserted key
	 * 1- the child splitted/0- didn't 
	 * median value of the child who splitted (and got up to the node) 
	 */
	...
	public void insert(T value) {
		...
		if (root == null) {
			...
			root.addKey(value);
			// push 0,0 because child node doesn't need to get split
			stack.push((T) ZERO);
			stack.push((T) ZERO);
		} 
		else {
			...
			while (currentNode != null && !wasAdded) {
				...
				if (currentNode.getNumberOfKeys() == maxDegree - 1) {											
					//save in the stack the index of the key that is going to rise up for the backtrack operation
					...
					//insert into queue 1- if the child of current node was splitted, otherwise 0 
					stack.push(medianval);
					stack.push((T) ONE);
				}
				else {
					if(!currentNode.equals(root)) { //root does not have children yet
						// push 0,0 because child node doesn't need to get split
						stack.push((T) ZERO);
						stack.push((T) ZERO);
					}
		...
		stack.push((T) value); //push the last added key
		++size;
	}

	/**
	 * delete the keyparent from the current node and merge it with his children
	 * @param NodeWithMed - the node with the key which we want to merge it's children with
	 * @param idxMed - the index of the parent of the children to merge (was median in the child)
	 * @param valmed - the value of the key who was median in the splitted child
	 */
	protected void merge(Node <T> NodeWithMed,Integer idxMed,T valmed) {
		Node <T> leftChild= NodeWithMed.getChild(idxMed);
		Node <T> rightChild= NodeWithMed.getChild(idxMed+1);
		leftChild.addKey(valmed);//add the key of the parent to the left child
		for (int i = 0; i < rightChild.numOfKeys; i++)  //copy the keys from right sibling to the ,merged node
			leftChild.addKey(rightChild.keys[i]);
		for (int i = 0; i < rightChild.numOfChildren; i++) //copy the children from right sibling to the merged node
			leftChild.addChild(rightChild.getChild(i));
		NodeWithMed.removeChild(idxMed+1); //remove who was the right child
		NodeWithMed.removeKey(idxMed); //remove the median that got up to the parent node
		if(root.numOfKeys==0) //if we merged the root, update the child to be the root
